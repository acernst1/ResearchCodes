...
void DSelector_kpkpxim_wf::Init(TTree *locTree)
{
	...
	dOutputFileName = "kpkpxim_wf.root"; //"" for none
	...
	dHist_XiMass024->Sumw2();
	dHist_XiMass029->Sumw2();
	dHist_XiMass034->Sumw2();
	dHist_XiMass039->Sumw2();
	dHist_XiMass044->Sumw2();
	dHist_XiMass049->Sumw2();
	dHist_XiMass054->Sumw2();
	dHist_XiMass059->Sumw2();
	dHist_XiMass064->Sumw2();
	dHist_XiMass069->Sumw2();
	dHist_XiMass074->Sumw2();
	dHist_XiMass079->Sumw2();
	dHist_XiMass084->Sumw2();
	dHist_XiMass089->Sumw2();
	dHist_XiMass094->Sumw2();
	dHist_XiMass099->Sumw2();
	dHist_XiMass104->Sumw2();
	dHist_XiMass109->Sumw2();
	dHist_XiMass114->Sumw2();
	dHist_XiMass024_acc->Sumw2();
	dHist_XiMass029_acc->Sumw2();
	dHist_XiMass034_acc->Sumw2();
	dHist_XiMass039_acc->Sumw2();
	dHist_XiMass044_acc->Sumw2();
	dHist_XiMass049_acc->Sumw2();
	dHist_XiMass054_acc->Sumw2();
	dHist_XiMass059_acc->Sumw2();
	dHist_XiMass064_acc->Sumw2();
	dHist_XiMass069_acc->Sumw2();
	dHist_XiMass074_acc->Sumw2();
	dHist_XiMass079_acc->Sumw2();
	dHist_XiMass084_acc->Sumw2();
	dHist_XiMass089_acc->Sumw2();
	dHist_XiMass094_acc->Sumw2();
	dHist_XiMass099_acc->Sumw2();
	dHist_XiMass104_acc->Sumw2();
	dHist_XiMass109_acc->Sumw2();
	dHist_XiMass114_acc->Sumw2();
	...
	myfile = new ofstream("XiEventNumbers_trackid_wf_ver03_ver20.txt");
	*myfile << "Run | Event | Mass | p Track id | pi1 Track id | pi2 Track id | s | Egamma" << endl;
...
Bool_t DSelector_kpkpxim_wf::Process(Long64_t locEntry)
{
	...
	*/
	...
	//Definition of Angles
		...
		*/
		...
		//E.g.  Print out event numbers 
		map<Particle_t, set<Int_t> > locUsedThisCombo_Eventlist;
		int locEventNumberXi = dComboWrapper -> Get_EventNumber();
		int locRunNumberXi = dComboWrapper -> Get_RunNumber();
		int loctrackpi1 = dPiMinus1Wrapper->Get_TrackID();
		int loctrackpi2= dPiMinus2Wrapper->Get_TrackID();
		int loctrackp = dProtonWrapper->Get_TrackID();
		double loc_s = (locBeamP4 + dTargetP4).M2();
		locUsedThisCombo_Eventlist[PiMinus].insert(locPiMinus1TrackID);
		locUsedThisCombo_Eventlist[PiMinus].insert(locPiMinus2TrackID);
		locUsedThisCombo_Eventlist[Proton].insert(locProtonTrackID);
		if(locUsedSoFar_Eventlist.find(locUsedThisCombo_Eventlist) == locUsedSoFar_Eventlist.end()){
			if(fabs(locDeltaT) < 6.004) {
					if(locXiP4_Measured.M() > 1.31 && locXiP4_Measured.M() < 1.34){
						*myfile <<  locRunNumberXi  << "  " <<  locEventNumberXi  << " " <<  locXiP4_Measured.M() << " "<< loctrackp << " " << loctrackpi1 << " " << loctrackpi2 << " " << loc_s << " " << locBeamP4.E() << endl;
					}
			}
			locUsedSoFar_Eventlist.insert(locUsedThisCombo_Eventlist);
		}
...
void DSelector_kpkpxim_wf::Finalize(void)
{
