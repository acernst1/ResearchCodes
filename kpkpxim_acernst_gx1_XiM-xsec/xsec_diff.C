...
//Perform accidental subtraction for signal histogram
	...
	sprintf(XiMasshistnameacc,"Xi_Egamma_t_%03d_wacc",binning);	// Accidentals here are scaled prior to being placed in the original histogram (X_wacc)
...
//Perform accidental subtraction for mc histogram
	...
	sprintf(MCXiMasshistnameacc,"Xi_Egamma_t_%03d_acc",binning); // Accidentals here are not scaled in the original histogram (X_acc)	
...
//Main beam energy loop
	...
	// Create a histogram of signal events vs t for this particular energy bin	
	...
	// Create a histogram of mc events vs t for this particular energy bin	
	...
	// Create a histogram of signal events for this particular energy bin
		XiMassKinFit_Egamma_t_accsub->GetZaxis()->SetRange(1,200);	// To integrate over approximately the full t range
		XiMassKinFit_Egamma_t_accsub->GetYaxis()->SetRange(Ebinmin,Ebinmax);
	...
	// Create a histogram of mc events for this particular energy bin	
		MC_XiMassKinFit_Egamma_t_accsub->GetZaxis()->SetRange(1,200);	// To integrate over approximately the full t range
	...
	//Main t loop
		...
		//Only perform signal fit if there are at least 20 entries in the whole histogram
			...
			else{ //Set up and perform signal fit for a particular t bin	
				Xi_canvas->cd();
		...
		//Set up and perform mc fit for a particular t bin
			Xi_mc_canvas->cd();
